...
env = PresinaEnv(hand_size=3, num_players=4)
...
def obs_to_state(obs):
    # Convert observation dict to a tuple for Q-table key
    return (
        tuple(obs['hand']),
        tuple(obs['played']),
        tuple(obs['predictions']),
        obs['phase'],
        obs['takes']
    )
...
def mc_control_epsilon_greedy(env, num_episodes, discount_factor=1.0, epsilon=0.1):
    ...
    Q_pred = defaultdict(lambda: np.zeros(50))  # max possible actions per state
    Q_play = defaultdict(lambda: np.zeros(50))
...
# TRAIN
Q_pred, Q_play, policy = mc_control_epsilon_greedy(env, num_episodes=1000, epsilon=0.1)
